...
import com.anaamalais.salescrm.List.TitleUpdateList;
import com.anaamalais.salescrm.List.Titles;
import com.anaamalais.salescrm.R;

import java.util.List;

public class TitleUpdateTypeArrayAdapter extends ArrayAdapter<String> {

    private final LayoutInflater mInflater;
    final Context mContext;
    private final List<TitleUpdateList> items;
    private final int mResource;
    TextView offTypeTv;

    public TitleUpdateTypeArrayAdapter(Context context, int resource,
                                 List objects) {
    ...
    private View createItemView(int position, View convertView, ViewGroup parent) {
        ...
        TitleUpdateList offerData = items.get(position);
        if (position == 0) {
