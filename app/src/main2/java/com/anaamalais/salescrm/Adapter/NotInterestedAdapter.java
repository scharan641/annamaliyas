...
public class NotInterestedAdapter extends RecyclerView.Adapter<NotInterestedAdapter.NotInterestedViewHolder>{
    //this context we will use to inflate the activity_sign_in
    private Context mCtx;
    //we are storing all the products in a list
    private List<NotInterestedList> notInterestedLists;

    String followupdate , followuptime;

    //getting the context and product list with constructor
    public NotInterestedAdapter(Context mCtx, List<NotInterestedList> notInterestedLists) {
        this.mCtx = mCtx;
        this.notInterestedLists = notInterestedLists;
    }

    @Override
    public NotInterestedViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.adapter_contact_followup, null);

        return new NotInterestedViewHolder(view);
    }

    @Override
    public void onBindViewHolder (final NotInterestedViewHolder holder, int position){
        //getting the product of the specified position
        final NotInterestedList allContactsList = notInterestedLists.get(position);
    ...
    public int getItemCount () {
        return notInterestedLists.size();
    }


    class NotInterestedViewHolder extends RecyclerView.ViewHolder {

        TextView txt_count, txt_call_time, txt_type, txt_remark , txt_today_remark;

        public NotInterestedViewHolder(View itemView) {
            super(itemView);
