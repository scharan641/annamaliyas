...
public class InvoiceCompletedAdapter extends RecyclerView.Adapter<InvoiceCompletedAdapter.InvoiceCompletedViewHolder> {


    //this context we will use to inflate the activity_sign_in
    private Context mCtx;
    //we are storing all the products in a list
    private List<InvoiceCompletedList> invoiceCompletedLists;

    String followupdate , followuptime;

    //getting the context and product list with constructor
    public InvoiceCompletedAdapter(Context mCtx, List<InvoiceCompletedList> invoiceCompletedLists) {
        this.mCtx = mCtx;
        this.invoiceCompletedLists = invoiceCompletedLists;
    }

    @Override
    public InvoiceCompletedViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.adapter_contact_followup, null);

        return new InvoiceCompletedViewHolder(view);
    }

    @Override
    public void onBindViewHolder (final InvoiceCompletedViewHolder holder, int position){
        //getting the product of the specified position
        final InvoiceCompletedList allContactsList = invoiceCompletedLists.get(position);
    ...
    public int getItemCount () {
        return invoiceCompletedLists.size();
    }


    class InvoiceCompletedViewHolder extends RecyclerView.ViewHolder {

        TextView txt_count, txt_call_time, txt_type, txt_remark , txt_today_remark;

        public InvoiceCompletedViewHolder(View itemView) {
            super(itemView);
