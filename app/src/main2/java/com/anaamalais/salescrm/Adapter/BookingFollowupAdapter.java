...
public class BookingFollowupAdapter extends RecyclerView.Adapter<BookingFollowupAdapter.BookingFollowupViewHolder> {


    //this context we will use to inflate the activity_sign_in
    private Context mCtx;
    //we are storing all the products in a list
    private List<BookingFollowupList> bookingFollowupLists;

    String followupdate , followuptime;

    //getting the context and product list with constructor
    public BookingFollowupAdapter(Context mCtx, List<BookingFollowupList> bookingFollowupLists) {
        this.mCtx = mCtx;
        this.bookingFollowupLists = bookingFollowupLists;
    }

    @Override
    public BookingFollowupViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.adapter_contact_followup, null);

        return new BookingFollowupViewHolder(view);
    }

    @Override
    public void onBindViewHolder (final BookingFollowupViewHolder holder, int position){
        //getting the product of the specified position
        final BookingFollowupList allContactsList = bookingFollowupLists.get(position);
    ...
    public int getItemCount () {
        return bookingFollowupLists.size();
    }


    class BookingFollowupViewHolder extends RecyclerView.ViewHolder {

        TextView txt_count, txt_call_time, txt_type, txt_remark , txt_today_remark;

        public BookingFollowupViewHolder(View itemView) {
            super(itemView);
