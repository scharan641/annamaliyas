...
public class OneKFollowupAdapter extends RecyclerView.Adapter<OneKFollowupAdapter.OneKFollowupViewHolder> {

    //this context we will use to inflate the activity_sign_in
    private Context mCtx;
    //we are storing all the products in a list
    private List<OneKFollowupList> oneKFollowupLists;

    String followupdate , followuptime;

    //getting the context and product list with constructor
    public OneKFollowupAdapter(Context mCtx, List<OneKFollowupList> oneKFollowupLists) {
        this.mCtx = mCtx;
        this.oneKFollowupLists = oneKFollowupLists;
    }

    @Override
    public OneKFollowupViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        View view = inflater.inflate(R.layout.adapter_contact_followup, null);

        return new OneKFollowupViewHolder(view);
    }

    @Override
    public void onBindViewHolder (final OneKFollowupViewHolder holder, int position){
        //getting the product of the specified position
        final OneKFollowupList allContactsList = oneKFollowupLists.get(position);
    ...
    public int getItemCount () {
        return oneKFollowupLists.size();
    }


    class OneKFollowupViewHolder extends RecyclerView.ViewHolder {

        TextView txt_count, txt_call_time, txt_type, txt_remark , txt_today_remark;

        public OneKFollowupViewHolder(View itemView) {
            super(itemView);
